/*
    ...
    Your Name: Bradley Allen

    Your Collaborators: Zach Sutton, Michael Murray
...
public class Automata {
	...
	// char output = applyRule(pat);
		...
		char[] newprev = new char[prev.length];
	...
	// String myString = "" + ch1 + ch2 + ch3; // works
		String next = "";
		if (idx == 0) {
			next = "" + prev[prev.length-1] + prev[0] + prev[1];
		}
		else {
			if (idx == prev.length - 1) {
				next = "" + prev[idx-1] + prev[idx] + prev[0];
			}
			else {
				next = "" + prev[idx-1] + prev[idx] + prev[idx+1];
			}
		}
	
		return next; // change this to return something reasonable
    }
